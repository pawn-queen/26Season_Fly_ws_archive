...
class OffboardControl(Node):
    ...
    def __init__(self,args) -> None:
        ...
        # 广角相机外参：机体坐标系采用 FRD（前、右、下）。
        # 安装角是相对于默认向下安装姿态、绕机体 FRD 三轴的补偿角。
 ...
 # 示例值，请务必替换成你自己的测量结果！
        ...
        self.target_timeout_duration = 0.7  # 目标信息超时秒数，例如1秒。可以设为命令行参数。
    ...
    def _capture_widecam_pose_snapshot(self):
        ...
        if not getattr(position, 'xy_global', True) and not self.widecam_local_reference_warned:
            self.get_logger().warn(
                "PX4 xy_global is false: wide-camera targets are only valid in the EKF local NED frame, "
                "not as WGS84 global coordinates."
            )
    ...
    def manage_dropping_sequence(self, drop_number: int) -> bool:
        ...
        if state == DroppingState.STEP_1_COMMANDED:
            ...
            self.servo_control.publish_dual_actuator_command(0.0, 0.0)  # ← 改用同步控制
    ...
    def fly_to_position_FRD2NED(self,x,y,z):
        ...
        '''
        x_target, y_target = self._frd_to_ned_at_pose(
            x, y, self.initial_x, self.initial_y, self.init_yaw
        )
        z_target = z
    ...
    @staticmethod
    def _frd_to_ned_at_pose(forward_m, right_m, origin_north_m, origin_east_m, yaw_rad):
        """Rotate a horizontal FRD offset into absolute local-NED coordinates."""
        north_m = forward_m * math.cos(yaw_rad) - right_m * math.sin(yaw_rad)
        east_m = forward_m * math.sin(yaw_rad) + right_m * math.cos(yaw_rad)
        return origin_north_m + north_m, origin_east_m + east_m
    ...
    def coordinate_FRD2NED(self,x,y):
        ...
        将FRD坐标转换为NED坐标。
        '''
        return self._frd_to_ned_at_pose(x, y, self.initial_x, self.initial_y, self.init_yaw)

    def coordinate_current_FRD2NED(self, x, y, origin_x=None, origin_y=None, yaw=None):
        '''
        将当前机体水平FRD坐标转换为NED坐标。
        用于相机单帧测量：平移应使用拍摄当下的飞机位置，旋转应使用当前航向。
        '''
        if origin_x is None:
        ...
        return self._frd_to_ned_at_pose(x, y, origin_x, origin_y, yaw)
    ...
    def _collect_global_search_map_sample(self, vision_info, sample_pose):
        """
        将当前视觉帧中的投水目标坐标转换为NED后存入多帧建图缓存。
        sample_pose must be the pose captured with the image, not the newest
        pose available when the vision timer finishes inference.
        """
        ...
        if self.widecam_map_reset_state is None:
            ...
            self.get_logger().warn(
                "PX4 local-position or heading reset detected during wide-camera mapping; "
        ...
        if (horizontal_speed is not None and
        ...
        frame_map = {}
        sample_x = sample_pose['x']
        sample_y = sample_pose['y']
        sample_yaw = sample_pose['yaw']
    ...
    def _calculate_and_store_average_map(self):
        ...
        for target_name in self.target_priority:
            if target_name in self.world_target_coordinates_ned:
                ...
                self.get_logger().info(f"  -> 已规划平均目标 '{target_name}' @ NED({ned_x:.2f}, {ned_y:.2f})")
            else:
                self.get_logger().warn(f"  -> 用户指定的目标 '{target_name}' 没有足够的平均坐标，将跳过。")
            ...
            if self.is_FinishDrop:
                if self.mission_state.value < MissionState.TRANSIT_TO_RECON_OFFBOARD.value:
                    ...
                    if self.timeout_2nd_postdrop is None:
                        ...
                        elapsed_time = (self.get_clock().now() - self.timeout_2nd_postdrop).nanoseconds / 1e9
                        if elapsed_time > self.post_drop_delay:
                    #回到投放区中心
                            self.publish_position_setpoint(self.DropArea_x,self.DropArea_y,self.takeoff_target_height)
...
def main(args=None) -> None:
    ...
    # 添加你想通过命令行配置的参数
    parser.add_argument('--model-path', type=str, default='/home/weights/8n_white_cylinder_detection_prime_anchor_fp16_20260708_122517.engine',
